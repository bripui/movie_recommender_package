...
def neighbor_recommender(user):
    ...
    recommendations = neighborhood.groupby(
        'movieId')['rating'].sum().sort_values(ascending=False)
    # remove already watched movies
    item_filter = (~recommendations.index.isin(
        list(user_with_ids['ratings'].keys())))
